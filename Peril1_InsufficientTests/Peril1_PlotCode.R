...
pspec <- c(0.75, 0.85, 0.75, 0.65, 0.55)
pprev <- 0.3
nbadgers <- 500
...
saveRDS(run, file = "Peril1_InsufficientTests/run3.rds")
...
r1_samp <- r1_samp %>%
  ...
  mutate(Truth = rep(c(0.3, 0.9, 0.75), times = 428400/3)) %>%
...
r2_samp <- r2_samp %>%
  ...
  mutate(Truth = rep(c(0.3, 0.9, 0.75), times = 428400/3)) %>%
  mutate(Mean = rep(c(r2_sum[1], r2_sum[2], r2_sum[3]), times = 428400/3))
...
r3_samp <- r3_samp %>%
  ...
  select(Inferred_parameter, Estimate, NumberOfTests) %>%
  mutate(Truth = rep(c(0.3, 0.9, 0.75), times = 428400/3)) %>%
  mutate(Mean = rep(c(r3_sum[1], r3_sum[2], r3_sum[3]), times = 428400/3))
...
r4_samp <- r4_samp %>%
  ...
  select(Inferred_parameter, Estimate, NumberOfTests) %>%
  mutate(Truth = rep(c(0.3, 0.9, 0.75), times = 428400/3)) %>%
  mutate(Mean = rep(c(r4_sum[1], r4_sum[2], r4_sum[3]), times = 428400/3))
...
r5_samp <- r5_samp %>%
  ...
  select(Inferred_parameter, Estimate, NumberOfTests) %>%
  mutate(Truth = rep(c(0.3, 0.9, 0.75), times = 428400/3)) %>%
  mutate(Mean = rep(c(r5_sum[1], r5_sum[2], r5_sum[3]), times = 428400/3))

data <- rbind(r1_samp, r2_samp, r3_samp, r4_samp, r5_samp)
data$NumberOfTests <- as.factor(data$NumberOfTests)
data <- data %>%
  mutate(Inferred_parameter = rep(c("Prevalence", "Sensitivity", "Specificity"), times = 2142000/3))
