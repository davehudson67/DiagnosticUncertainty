...
summaryprobinference<-array(0,dim=c(reps,length(psensvec)))
summaryprobinfsd<-array(0,dim=c(reps,length(psensvec)))
...
system.time(runprob <- runMCMC(cMCMC, 
                           niter = 50000, 
                           nburnin = 2400, 
                           nchains = 3, 
                           progressBar = TRUE, 
...
#plot(run3$samples)
runprobsummary<-MCMCsummary(runprob$samples)
summaryprobinference[r,q]<-runprobsummary$mean[2]
summaryprobinfsd[r,q]<-runprobsummary$sd[2]
}
}

summaryprobinference2<-summaryprobinference
summaryprobinfsd2<-summaryprobinfsd

plot(summaryprobinference2[,2]~psensvec,type="n",ylim=c(0.6,1))
abline(0,1)
points(summaryprobinference2[,2]~psensvec,pch=16,cex=2,col="black")
points(summaryprobinference1[,2]~psensvec,pch=16,cex=2,col="black")
points(1/(1+1/exp(summarylogitinference2[,2]))~psensvec,pch=16,cex=2,col="red")
points(1/(1+1/exp(summarylogitinference1[,2]))~psensvec,pch=16,cex=2,col="red")
