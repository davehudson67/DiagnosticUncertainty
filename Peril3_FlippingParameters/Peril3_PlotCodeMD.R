...
for(run_number in 1:5) {
  ...
  # Run the MCMC
  system.time(run <- runMCMC(cMCMC, niter = 500000, nburnin = 24000, nchains = 3, progressBar = TRUE, summary = TRUE, samplesAsCodaMCMC = TRUE, thin = 1))
  
  # Save the output to a unique file
  file_name <- paste0("Peril3_FlippingParameters/run", run_number, ".rds")
...
for (i in 1:n_runs) {
  ...
  filename <- paste0("Peril3_FlippingParameters/run", i, ".rds")
